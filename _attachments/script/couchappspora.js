...
function getPostUpdates( opts ) {
  var posts, comments;

  function bindDbChanges(database, callback) {
    $.getJSON("/" + database, function(db) {
	  $.getJSON("/"+ database + "/_changes?since="+ (db.update_seq) +"&heartbeat=10000&feed=longpoll", 
	    function(changes) {
		  if($.isFunction(callback)){ 
		    callback.call(this, changes);
		    bindDbChanges(database, callback);
		  }
		}
	  );
	});
  ...
  bindDbChanges("couchappspora", function(changes){
    var rows = [];
	var keys = [];
console.log(changes);
	$(changes.results).each(function(index, el) {
//		keys.push(el.id);
//console.log(el.created_at.toString());
		$.couch.db( config.db ).view( config.design + '/stream', {
			"descending" : true,
		    "limit" : 20,
		    success: function( data ) {
			    posts = data;
//console.log(posts);
			    render( 'stream', 'stream', posts, 'prepend' );
		    }
		});
	});
  });
...
function bindUpdater() {
	setInterval(getPostUpdates, config.refreshRate);
}
